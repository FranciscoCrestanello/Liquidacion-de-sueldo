...
void Empleado::cargar(){
    ...
    while(verificarEdad(d,m,a) || !(validarIngreso(fechaIngreso)) ){
            locate(4,8);cout<<"!";
...
bool validarIngreso(Fecha obj){
    Fecha aux;
    if(obj.getDia()<=aux.getDia()){
        if(obj.getMes()<=aux.getMes()){
            if(obj.getAnio()<=aux.getAnio()){
                return true;
            }
        }
    }
